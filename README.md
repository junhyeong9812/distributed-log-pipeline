# Distributed Log Pipeline

> 분산처리 시스템(Kubernetes, Hadoop, Spark)을 학습하기 위한 실습 프로젝트

---

## 📋 프로젝트 개요

### 목적
Kubernetes, Hadoop, Spark를 활용한 분산 데이터 파이프라인을 직접 구축하고 운영해보며 분산처리 시스템의 핵심 개념을 학습합니다.

### 주요 기능
- 배치 기반 로그/이벤트 데이터 자동 생성
- 실시간 데이터 수집 및 메시지 큐 처리
- 분산 저장소(HDFS)에 데이터 적재
- Spark를 활용한 실시간/배치 데이터 분석
- Grafana 기반 모니터링 대시보드

---

## 🏗️ 시스템 아키텍처

### 전체 구조

```
┌─────────────────────────────────────────────────────────────────────┐
│                         System Architecture                          │
└─────────────────────────────────────────────────────────────────────┘

┌─────────────────┐       ┌─────────────────────────────────────────┐
│                 │       │            리눅스 우분투 - 노트북 (Master)               │
│   Data          │       │                                         │
│   Generator     │       │  ┌───────────────────────────────────┐  │
│   Server        │ ────▶ │  │   Java Backend (Spring Boot)      │  │
│                 │ HTTP  │  │   • REST API 수집 서버             │  │
│  (Python)       │       │  │   • Kafka Producer                │  │
│  • FastAPI      │       │  └───────────────────────────────────┘  │
│  • 배치 스케줄러 │       │                    │                    │
│  • 데이터 생성기 │       │                    ▼                    │
│                 │       │  ┌───────────────────────────────────┐  │
└─────────────────┘       │  │   Kubernetes Control Plane        │  │
                          │  │   • API Server                    │  │
                          │  │   • Scheduler                     │  │
                          │  │   • etcd                          │  │
                          │  └───────────────────────────────────┘  │
                          │                    │                    │
                          │  ┌───────────────────────────────────┐  │
                          │  │   Kafka (KRaft 모드)               │  │
                          │  │   • 메시지 큐                      │  │
                          │  │   • 실시간 데이터 버퍼링           │  │
                          │  └───────────────────────────────────┘  │
                          │                    │                    │
                          │  ┌───────────────────────────────────┐  │
                          │  │   Spark Driver + Airflow          │  │
                          │  │   • 작업 스케줄링                  │  │
                          │  │   • 분석 Job 관리                  │  │
                          │  └───────────────────────────────────┘  │
                          │                    │                    │
                          │  ┌───────────────────────────────────┐  │
                          │  │   Monitoring Stack                │  │
                          │  │   • Grafana (시각화)              │  │
                          │  │   • Prometheus (메트릭 수집)       │  │
                          │  └───────────────────────────────────┘  │
                          └─────────────────────────────────────────┘
                                              │
                          ┌───────────────────┴───────────────────┐
                          │                                       │
                          ▼                                       ▼
               ┌─────────────────────┐             ┌─────────────────────┐
               │   리눅스 PC - A      │             │   리눅스 PC - B      │
               │   (Worker Node 1)   │             │   (Worker Node 2)   │
               │                     │             │                     │
               │ ┌─────────────────┐ │             │ ┌─────────────────┐ │
               │ │ K8s Worker      │ │             │ │ K8s Worker      │ │
               │ │ (kubelet)       │ │             │ │ (kubelet)       │ │
               │ └─────────────────┘ │             │ └─────────────────┘ │
               │                     │             │                     │
               │ ┌─────────────────┐ │             │ ┌─────────────────┐ │
               │ │ HDFS DataNode   │ │◀───────────▶│ │ HDFS DataNode   │ │
               │ │ (데이터 저장)    │ │  Replication│ │ (데이터 저장)    │ │
               │ └─────────────────┘ │             │ └─────────────────┘ │
               │                     │             │                     │
               │ ┌─────────────────┐ │             │ ┌─────────────────┐ │
               │ │ Spark Executor  │ │             │ │ Spark Executor  │ │
               │ │ (데이터 처리)    │ │             │ │ (데이터 처리)    │ │
               │ └─────────────────┘ │             │ └─────────────────┘ │
               └─────────────────────┘             └─────────────────────┘
```

### 데이터 플로우

```
┌─────────────────────────────────────────────────────────────────────┐
│                          Data Flow                                   │
└─────────────────────────────────────────────────────────────────────┘

1. 데이터 생성 (Python Generator)
   │
   │  배치 스케줄 (매 N초/분마다)
   │  • 시스템 로그 생성
   │  • 사용자 활동 이벤트 생성
   ▼
2. 데이터 수집 (Java Spring Boot)
   │
   │  REST API로 수신
   │  • 데이터 검증
   │  • Kafka로 발행
   ▼
3. 메시지 큐 (Kafka)
   │
   ├──────────────────────┬──────────────────────┐
   │                      │                      │
   ▼                      ▼                      ▼
4a. 실시간 처리       4b. 배치 저장          4c. HDFS 적재
    (Spark Streaming)     (Kafka → HDFS)         (원본 보관)
    │                      │                      │
    │ • 5초 윈도우 집계     │ • 시간별 파티션      │ • 장기 보관
    │ • 이상 탐지          │ • 압축 저장          │ • 재처리용
    ▼                      ▼                      ▼
5. 결과 저장
   │
   ├── Prometheus (메트릭)
   ├── HDFS (분석 결과)
   └── PostgreSQL (집계 데이터)
   │
   ▼
6. 시각화 (Grafana)
   │
   • 실시간 대시보드
   • 알림 설정
```

---

## 🖥️ PC별 역할 및 구성

### 역할 분배

| PC | 역할 | 주요 컴포넌트 | 권장 사양 |
|-----|------|-------------|----------|
| **노트북** | Master + Generator | K8s Control Plane, Java Backend, Kafka, Spark Driver, Monitoring | 8GB+ RAM, 4+ Core |
| **리눅스 A** | Worker Node 1 | K8s Worker, HDFS DataNode, Spark Executor | 4GB+ RAM, 2+ Core |
| **리눅스 B** | Worker Node 2 | K8s Worker, HDFS DataNode, Spark Executor | 4GB+ RAM, 2+ Core |

### 네트워크 구성

```
┌─────────────────────────────────────────────────────────┐
│                    Network Topology                      │
└─────────────────────────────────────────────────────────┘

         192.168.x.0/24 (예시)
                  │
    ┌─────────────┼─────────────┐
    │             │             │
    ▼             ▼             ▼
┌───────┐    ┌───────┐    ┌───────┐
│Master │    │Worker1│    │Worker2│
│  .10  │    │  .11  │    │  .12  │
└───────┘    └───────┘    └───────┘

필요 포트:
• 6443  : Kubernetes API
• 9092  : Kafka
• 9870  : HDFS NameNode Web UI
• 8080  : Spark Master Web UI
• 3000  : Grafana
• 8081  : Java Backend API
```

---

## 📁 프로젝트 구조

```
distributed-log-pipeline/
│
├── README.md
├── docker-compose.yml              # 로컬 테스트용
├── .env.example                    # 환경변수 템플릿
│
├── docs/                           # 문서
│   ├── SETUP_MASTER.md            # 마스터 노드 설정 가이드
│   ├── SETUP_WORKER.md            # 워커 노드 설정 가이드
│   ├── TROUBLESHOOTING.md         # 문제 해결 가이드
│   └── ARCHITECTURE.md            # 상세 아키텍처 문서
│
├── generator/                      # 데이터 생성기 (Python)
│   ├── Dockerfile
│   ├── requirements.txt
│   ├── app/
│   │   ├── main.py                # FastAPI 엔트리포인트
│   │   ├── scheduler.py           # 배치 스케줄러
│   │   ├── generators/
│   │   │   ├── log_generator.py   # 로그 데이터 생성
│   │   │   └── event_generator.py # 이벤트 데이터 생성
│   │   └── config.py              # 설정
│   └── tests/
│
├── backend/                        # 수집 서버 (Java Spring Boot)
│   ├── Dockerfile
│   ├── pom.xml
│   ├── src/
│   │   └── main/
│   │       ├── java/
│   │       │   └── com/pipeline/
│   │       │       ├── PipelineApplication.java
│   │       │       ├── controller/
│   │       │       │   └── CollectorController.java
│   │       │       ├── service/
│   │       │       │   └── KafkaProducerService.java
│   │       │       ├── model/
│   │       │       │   ├── LogEvent.java
│   │       │       │   └── ActivityEvent.java
│   │       │       └── config/
│   │       │           └── KafkaConfig.java
│   │       └── resources/
│   │           └── application.yml
│   └── tests/
│
├── spark-jobs/                     # Spark 작업 (PySpark)
│   ├── streaming/
│   │   ├── realtime_aggregator.py # 실시간 집계
│   │   └── anomaly_detector.py    # 이상 탐지
│   ├── batch/
│   │   ├── daily_report.py        # 일별 리포트
│   │   └── weekly_analysis.py     # 주간 분석
│   └── common/
│       └── spark_utils.py         # 공통 유틸
│
├── airflow/                        # 워크플로우 스케줄링
│   ├── dags/
│   │   ├── daily_pipeline.py
│   │   └── weekly_pipeline.py
│   └── plugins/
│
├── kubernetes/                     # K8s 매니페스트
│   ├── namespace.yaml
│   ├── master/                    # 마스터 노드용
│   │   ├── kafka/
│   │   │   ├── zookeeper.yaml
│   │   │   └── kafka.yaml
│   │   ├── spark/
│   │   │   └── spark-master.yaml
│   │   └── monitoring/
│   │       ├── prometheus.yaml
│   │       └── grafana.yaml
│   ├── worker/                    # 워커 노드용
│   │   ├── hdfs/
│   │   │   └── datanode.yaml
│   │   └── spark/
│   │       └── spark-worker.yaml
│   └── common/                    # 공통
│       ├── configmaps.yaml
│       └── secrets.yaml
│
├── hadoop/                         # Hadoop 설정
│   ├── core-site.xml
│   ├── hdfs-site.xml
│   └── scripts/
│       ├── start-namenode.sh
│       └── start-datanode.sh
│
├── monitoring/                     # 모니터링 설정
│   ├── grafana/
│   │   ├── provisioning/
│   │   └── dashboards/
│   │       ├── overview.json      # 전체 현황 대시보드
│   │       ├── kafka.json         # Kafka 모니터링
│   │       └── spark.json         # Spark 모니터링
│   └── prometheus/
│       └── prometheus.yml
│
└── scripts/                        # 유틸리티 스크립트
    ├── setup-master.sh            # 마스터 초기 설정
    ├── setup-worker.sh            # 워커 초기 설정
    ├── deploy-all.sh              # 전체 배포
    └── cleanup.sh                 # 정리
```

---

## 🚀 빠른 시작

### 사전 요구사항

**모든 PC 공통:**
```bash
# Docker 설치
curl -fsSL https://get.docker.com | sh
sudo usermod -aG docker $USER

# kubectl 설치
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl"
sudo install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl
```

### Step 1: 프로젝트 클론 (모든 PC)

```bash
git clone https://github.com/your-repo/distributed-log-pipeline.git
cd distributed-log-pipeline
cp .env.example .env
# .env 파일에서 각 PC의 IP 주소 설정
```

### Step 2: Master 노드 설정 (노트북)

```bash
# 환경변수 설정
export NODE_ROLE=master
export MASTER_IP=192.168.x.10  # 실제 IP로 변경

# Kubernetes 클러스터 초기화
./scripts/setup-master.sh

# 마스터 컴포넌트 배포
kubectl apply -f kubernetes/master/

# Java Backend 실행
cd backend
./mvnw spring-boot:run

# 데이터 생성기 실행 (별도 터미널)
cd generator
pip install -r requirements.txt
python -m app.main
```

### Step 3: Worker 노드 설정 (리눅스 A, B)

```bash
# 환경변수 설정
export NODE_ROLE=worker
export MASTER_IP=192.168.x.10  # 마스터 IP

# 클러스터 조인
./scripts/setup-worker.sh

# 워커 컴포넌트 배포
kubectl apply -f kubernetes/worker/
```

### Step 4: 동작 확인

```bash
# 클러스터 상태 확인
kubectl get nodes
kubectl get pods -A

# HDFS 상태 확인
hdfs dfsadmin -report

# Kafka 토픽 확인
kafka-topics.sh --list --bootstrap-server localhost:9092

# Grafana 접속
# 브라우저에서 http://<MASTER_IP>:3000 접속
# 기본 계정: admin / admin
```

---

## 📊 생성 데이터 형식

### 시스템 로그

```json
{
  "timestamp": "2024-01-15T10:30:00.123Z",
  "level": "INFO",
  "service": "api-gateway",
  "host": "worker-1",
  "message": "Request processed successfully",
  "metadata": {
    "request_id": "uuid-1234",
    "response_time_ms": 45,
    "status_code": 200,
    "endpoint": "/api/users",
    "method": "GET"
  }
}
```

### 사용자 활동 이벤트

```json
{
  "event_id": "evt-uuid-5678",
  "timestamp": "2024-01-15T10:30:05.456Z",
  "user_id": "user_12345",
  "session_id": "sess_abcde",
  "event_type": "CLICK",
  "event_data": {
    "page": "/products/123",
    "element": "add_to_cart_button",
    "position": {"x": 450, "y": 320}
  },
  "device": {
    "type": "mobile",
    "os": "iOS",
    "browser": "Safari"
  }
}
```

---

## 📈 모니터링 대시보드

### 제공되는 대시보드

| 대시보드 | 설명 | 주요 메트릭 |
|---------|------|------------|
| **Overview** | 시스템 전체 현황 | 노드 상태, 처리량, 에러율 |
| **Kafka** | 메시지 큐 모니터링 | 토픽별 처리량, 지연시간, 컨슈머 랙 |
| **Spark** | 처리 작업 모니터링 | 작업 상태, 처리 시간, 리소스 사용량 |
| **HDFS** | 저장소 모니터링 | 용량, 복제 상태, 노드 상태 |

### 알림 설정

```yaml
# 기본 제공 알림 규칙
alerts:
  - name: HighErrorRate
    condition: error_rate > 5%
    duration: 5m
    
  - name: KafkaLag
    condition: consumer_lag > 10000
    duration: 10m
    
  - name: SparkJobFailed
    condition: job_status == "FAILED"
```

---

## 🔧 주요 설정

### 배치 스케줄 설정 (generator/app/config.py)

```python
BATCH_CONFIG = {
    "log_interval_seconds": 5,      # 로그 생성 주기
    "event_interval_seconds": 10,   # 이벤트 생성 주기
    "batch_size": 100,              # 배치당 데이터 수
    "target_backend_url": "http://master:8081/api/collect"
}
```

### Kafka 토픽 설정

| 토픽 | 파티션 | 복제 계수 | 보존 기간 |
|------|--------|----------|----------|
| logs.raw | 3 | 2 | 7일 |
| events.activity | 3 | 2 | 7일 |
| alerts.realtime | 1 | 2 | 1일 |

---

## 🛠️ 개발 가이드

### 로컬 테스트 (Docker Compose)

```bash
# 전체 스택 로컬 실행 (단일 PC 테스트용)
docker-compose up -d

# 로그 확인
docker-compose logs -f

# 종료
docker-compose down -v
```

### 새로운 Spark Job 추가

```python
# spark-jobs/batch/my_new_job.py

from pyspark.sql import SparkSession
from common.spark_utils import get_spark_session, read_from_hdfs

def main():
    spark = get_spark_session("MyNewJob")
    
    # HDFS에서 데이터 읽기
    df = read_from_hdfs(spark, "/data/logs/2024/01/15")
    
    # 처리 로직
    result = df.groupBy("level").count()
    
    # 결과 저장
    result.write.mode("overwrite").parquet("/data/results/my_job")

if __name__ == "__main__":
    main()
```

---

## 📚 학습 로드맵

### Week 1-2: 환경 구축
- [ ] 3대 PC 네트워크 구성
- [ ] Docker, kubectl 설치
- [ ] Kubernetes 클러스터 구축
- [ ] 기본 통신 테스트

### Week 3: 데이터 파이프라인 기초
- [ ] Kafka 설치 및 토픽 생성
- [ ] Java Backend 개발
- [ ] Python Generator 개발
- [ ] 기본 데이터 플로우 확인

### Week 4: HDFS 연동
- [ ] HDFS 클러스터 구성
- [ ] Kafka → HDFS 연동
- [ ] 데이터 적재 확인

### Week 5: Spark 처리
- [ ] Spark Streaming 작업 개발
- [ ] Batch 작업 개발
- [ ] Airflow 스케줄링

### Week 6: 모니터링
- [ ] Prometheus + Grafana 구성
- [ ] 대시보드 구성
- [ ] 알림 설정
- [ ] 전체 통합 테스트

---

## ❓ FAQ

### Q: 단일 PC에서도 테스트할 수 있나요?
A: 네, `docker-compose up`으로 로컬에서 전체 스택을 테스트할 수 있습니다.

### Q: 최소 사양은 어떻게 되나요?
A: Master 8GB RAM, Worker 각 4GB RAM을 권장합니다. 더 낮은 사양에서도 동작하지만 성능이 제한될 수 있습니다.

### Q: Windows에서도 가능한가요?
A: WSL2 + Docker Desktop 환경에서 가능합니다. 단, 네트워크 설정이 복잡해질 수 있습니다.
---

## 🔧 기술 상세 설명

### PySpark는 어떻게 동작하나요?

Spark는 Java/Scala로 만들어졌지만, Python으로 작성한 코드도 실행할 수 있습니다.

#### 동작 구조
```
┌─────────────────────────────────────────────────────────┐
│                    PySpark 구조                          │
├─────────────────────────────────────────────────────────┤
│                                                          │
│   Python 코드 (우리가 작성)                               │
│        ↓                                                 │
│   Py4J (Python ↔ Java 브릿지)                            │
│        ↓                                                 │
│   Spark Core (Java/Scala - 실제 실행)                    │
│        ↓                                                 │
│   JVM에서 분산 처리                                       │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

#### 실제 실행 흐름
```python
# 우리가 작성한 Python 코드
df.filter(col("level") == "ERROR").count()
```

위 코드가 실행되면:

1. **Python → Py4J**: Python 코드가 Py4J 라이브러리를 통해 Java로 전달
2. **실행 계획 생성**: Spark(Java)가 최적화된 실행 계획 생성
3. **JVM 분산 처리**: 실제 데이터 처리는 JVM에서 분산 실행
4. **결과 반환**: 처리 결과만 Python으로 반환

#### 성능 비교

| 구분 | PySpark | Scala/Java Spark |
|------|---------|------------------|
| **DataFrame API** | 거의 동일 | 기준 |
| **RDD 연산** | 약간 느림 | 빠름 |
| **UDF (사용자 함수)** | 느림 (직렬화 오버헤드) | 빠름 |
| **개발 속도** | 빠름 | 느림 |

#### 언제 무엇을 선택할까?
```
DataFrame/SQL 위주 작업     → PySpark 추천 (충분한 성능)
복잡한 커스텀 로직/UDF 많음  → Scala 고려
ML 파이프라인               → PySpark (MLlib + Python 생태계)
```

본 프로젝트는 DataFrame API 위주로 작업하므로 **PySpark로 충분**합니다.

---

### Kafka 토픽 파티션 설정 주의사항

#### 문제 상황

Kafka 토픽이 의도한 파티션 수와 다르게 생성될 수 있습니다.
```
예상: 파티션 3개로 분산 저장
실제: 파티션 1개에 모든 데이터 저장
```

#### 원인
```
┌─────────────────────────────────────────────────────────┐
│              서비스 시작 순서 문제                        │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  Case 1: Generator가 먼저 데이터 전송                    │
│  → Kafka가 토픽 자동 생성 (AUTO_CREATE_TOPICS=true)      │
│  → 기본값 파티션 1개로 생성                              │
│  → Java Backend의 KafkaConfig 설정 무시됨               │
│                                                          │
│  Case 2: Java Backend가 먼저 연결                        │
│  → KafkaConfig.java의 NewTopic 설정 적용                │
│  → 파티션 3개로 토픽 생성                                │
│  → 데이터가 정상적으로 분산 저장                         │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

#### 해결 방법

**방법 1: 서비스 시작 순서 보장**
```yaml
# docker-compose.yml
generator:
  depends_on:
    backend:
      condition: service_healthy  # Backend가 healthy 상태일 때만 시작
```

**방법 2: 토픽 사전 생성**
```bash
# Kafka 컨테이너에서 직접 토픽 생성
docker exec kafka /opt/kafka/bin/kafka-topics.sh \
  --create \
  --topic logs.raw \
  --partitions 3 \
  --replication-factor 1 \
  --bootstrap-server localhost:9092
```

**방법 3: 볼륨 초기화 후 재시작**
```bash
docker compose down -v  # 볼륨까지 삭제
docker compose up -d    # 새로 시작
```

#### 파티션 분배 원리
```java
// KafkaProducerService.java
String key = logEvent.getService();  // key = service 이름
kafkaTemplate.send(logsTopic, key, logEvent);
```

| Key (Service) | 파티션 할당 (hash % 파티션수) |
|---------------|------------------------------|
| api-gateway | 0, 1, 또는 2 |
| user-service | 0, 1, 또는 2 |
| order-service | 0, 1, 또는 2 |

**같은 key는 항상 같은 파티션**으로 전송되어 메시지 순서가 보장됩니다.

---

### Spark Streaming 파티션 저장 문제 해결

#### 문제 현상

HDFS에 데이터가 저장될 때 파티션이 제대로 생성되지 않는 문제:
```
# 예상한 결과
/data/logs/raw/year=2026/month=1/day=11/hour=19/

# 실제 결과
/data/logs/raw/year=__HIVE_DEFAULT_PARTITION__/month=__HIVE_DEFAULT_PARTITION__/...
```

#### 원인

Generator가 보내는 timestamp 형식과 Spark에서 파싱하는 형식이 불일치:
```python
# Generator가 보내는 형식 (Unix timestamp)
{
    "timestamp": 1768123166.291045000,  # epoch seconds
    ...
}

# 처음 Spark 코드 (ISO 형식으로 파싱 시도)
.withColumn("event_time", to_timestamp(col("timestamp"), "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"))
# → 파싱 실패 → null → __HIVE_DEFAULT_PARTITION__
```

#### 해결

Unix timestamp를 올바르게 파싱하도록 수정:
```python
# 수정 전 (ISO 형식)
.withColumn("event_time", to_timestamp(col("timestamp"), "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"))

# 수정 후 (Unix timestamp)
.withColumn("event_time", from_unixtime(col("timestamp")).cast("timestamp"))
```

또한 스키마에서 timestamp 타입도 변경:
```python
# 수정 전
StructField("timestamp", StringType(), True)

# 수정 후
StructField("timestamp", DoubleType(), True)
```

#### 디버깅 방법
```bash
# 1. Generator가 보내는 실제 데이터 형식 확인
curl -s http://<MASTER_IP>:8000/sample/log

# 2. Kafka UI에서 메시지 직접 확인
# http://<MASTER_IP>:8080 → Topics → logs.raw → Messages
```

---

### Spark Job 리소스 점유 문제

#### 문제 현상

Spark Job 재실행 시 리소스를 할당받지 못하는 경고:
```
WARN TaskSchedulerImpl: Initial job has not accepted any resources; 
check your cluster UI to ensure that workers are registered and have sufficient resources
```

#### 원인

이전 Spark Job이 비정상 종료되면서 Worker의 리소스를 계속 점유하고 있음.
```
┌─────────────────────────────────────────────────────────┐
│                   리소스 점유 상태                        │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  이전 Job (종료됨)     현재 Job (대기중)                  │
│  └─ Worker 리소스 점유  └─ 리소스 할당 대기               │
│     (24 cores, 1GB)       (할당 불가)                    │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

#### 해결 방법

**방법 1: Spark 컨테이너 재시작**
```bash
docker compose restart spark-master spark-worker
```

**방법 2: Spark Master UI에서 직접 종료**
```
1. http://<MASTER_IP>:8082 접속
2. Running Applications에서 (kill) 클릭
3. 새 Job 실행
```

**방법 3: 전체 클러스터 재시작**
```bash
docker compose down
docker compose up -d
```

#### 예방 방법

Spark Job 종료 시 항상 `Ctrl+C`로 정상 종료하고, Spark UI에서 Application이 Completed로 변경되었는지 확인.

---

### Airflow 설정 및 트러블슈팅

#### Airflow 기본 정보

| 항목 | 내용 |
|------|------|
| **언어** | Python |
| **기본 DB** | SQLite (개발용) |
| **운영 DB** | PostgreSQL, MySQL (권장) |

#### 문제 1: DB 초기화 실패

**증상:**
```
ERROR: You need to initialize the database. Please run `airflow db init`.
```

**원인:**
- 이전 실행에서 DB가 불완전하게 생성됨
- `docker compose down`은 볼륨을 유지하므로 불완전한 DB가 남아있음

**해결:**
```bash
# 볼륨까지 삭제해서 깨끗하게 시작
docker compose down -v
docker compose up -d
```

#### 문제 2: SQLite + LocalExecutor 호환 불가

**증상:**
```
airflow.exceptions.AirflowConfigException: error: cannot use SQLite with the LocalExecutor
```

**원인:**
- SQLite는 동시 쓰기를 지원하지 않음
- LocalExecutor는 병렬 실행을 위해 동시 DB 접근 필요
- 따라서 SQLite + LocalExecutor 조합은 불가

**Executor 종류:**

| Executor | DB 요구사항 | 특징 |
|----------|------------|------|
| **SequentialExecutor** | SQLite 가능 | 순차 실행, 개발용 |
| **LocalExecutor** | PostgreSQL/MySQL 필요 | 병렬 실행 |
| **CeleryExecutor** | PostgreSQL/MySQL + Redis | 분산 실행 |
| **KubernetesExecutor** | PostgreSQL/MySQL | K8s Pod으로 실행 |

**해결:**
```yaml
# docker-compose.yml
environment:
  # 수정 전 (에러 발생)
  - AIRFLOW__CORE__EXECUTOR=LocalExecutor
  
  # 수정 후 (정상 동작)
  - AIRFLOW__CORE__EXECUTOR=SequentialExecutor
```

#### 문제 3: DB 명령어 버전 차이

**증상:**
```
DeprecationWarning: `db init` is deprecated. Use `db migrate` instead
```

**원인:**
- Airflow 2.7.0부터 `airflow db init`이 deprecated
- 새 버전에서는 `airflow db migrate` 사용 권장

**해결:**
```yaml
# 두 명령어 모두 실행하여 호환성 확보
command: bash -c "airflow db init && airflow users create ... "
```

#### 최종 Airflow 설정 (docker-compose.yml)
```yaml
airflow:
  image: apache/airflow:2.7.0-python3.10
  container_name: airflow
  user: root
  ports:
    - "8084:8080"
  environment:
    - AIRFLOW__CORE__EXECUTOR=SequentialExecutor  # SQLite와 호환
    - AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=sqlite:////opt/airflow/airflow.db
    - AIRFLOW__CORE__FERNET_KEY=46BKJoQYlPPOexq0OhDZnIlNepKFf87WFwLbfzqDDho=
    - AIRFLOW__CORE__LOAD_EXAMPLES=False
    - AIRFLOW__WEBSERVER__SECRET_KEY=mysecretkey123
  volumes:
    - ./airflow/dags:/opt/airflow/dags
    - ./spark-jobs:/opt/spark-jobs
    - /var/run/docker.sock:/var/run/docker.sock  # Docker 명령 실행용
  command: bash -c "airflow db init && airflow users create --username admin --password admin --firstname Admin --lastname User --role Admin --email admin@example.com || true && (airflow webserver &) && airflow scheduler"
```

#### Airflow 접속 정보
```
URL: http://<MASTER_IP>:8084
ID: admin
PW: admin
```

---

## 🖥️ 분산 환경 구성

### 시스템 아키텍처
```
┌─────────────────────────────────────────────────────────────────────┐
│                       분산 클러스터 구성                              │
└─────────────────────────────────────────────────────────────────────┘

    ┌─────────────────────────────────────────────────────────┐
    │             노트북 - Master (192.168.55.114)             │
    │                                                          │
    │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
    │  │  Kafka   │ │ NameNode │ │  Spark   │ │ Airflow  │   │
    │  │          │ │  (HDFS)  │ │  Master  │ │          │   │
    │  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
    │                                                          │
    │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
    │  │ Backend  │ │Generator │ │ Grafana  │ │Prometheus│   │
    │  │  (Java)  │ │ (Python) │ │          │ │          │   │
    │  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
    └─────────────────────────────────────────────────────────┘
                              │
              ┌───────────────┴───────────────┐
              │                               │
              ▼                               ▼
    ┌─────────────────────┐       ┌─────────────────────┐
    │ 리눅스 A - Worker 1  │       │ 리눅스 B - Worker 2  │
    │  (192.168.55.158)   │       │   (192.168.55.9)    │
    │                     │       │                     │
    │  ┌───────────────┐  │       │  ┌───────────────┐  │
    │  │   DataNode    │  │       │  │   DataNode    │  │
    │  │    (HDFS)     │  │       │  │    (HDFS)     │  │
    │  └───────────────┘  │       │  └───────────────┘  │
    │                     │       │                     │
    │  ┌───────────────┐  │       │  ┌───────────────┐  │
    │  │ Spark Worker  │  │       │  │ Spark Worker  │  │
    │  └───────────────┘  │       │  └───────────────┘  │
    └─────────────────────┘       └─────────────────────┘
```

### PC별 역할 및 서비스

| PC | IP | 역할 | 서비스 | 포트 |
|----|-----|------|--------|------|
| **노트북** | 192.168.55.114 | Master | Kafka | 9092 |
| | | | Kafka UI | 8080 |
| | | | HDFS NameNode | 9870, 9000 |
| | | | Spark Master | 8082, 7077 |
| | | | Airflow | 8084 |
| | | | Grafana | 3000 |
| | | | Prometheus | 9090 |
| | | | Backend | 8081 |
| | | | Generator | 8000 |
| **리눅스 A** | 192.168.55.158 | Worker 1 | HDFS DataNode | 9864 |
| | | | Spark Worker | 8081 |
| **리눅스 B** | 192.168.55.9 | Worker 2 | HDFS DataNode | 9864 |
| | | | Spark Worker | 8081 |

### 배포 파일 구조
```
deploy/
├── docker-compose.master.yml   # Master 노드용
└── docker-compose.worker.yml   # Worker 노드용

scripts/
├── setup-master.sh             # Master 초기 설정 스크립트
└── setup-worker.sh             # Worker 초기 설정 스크립트
```

### 빠른 시작

#### Master 노드 (노트북)
```bash
# 1. 설정 스크립트 실행 (최초 1회)
./scripts/setup-master.sh

# 2. Backend 빌드
cd backend && ./gradlew build && cd ..

# 3. 서비스 시작
cd deploy
docker compose -f docker-compose.master.yml up -d --build
```

#### Worker 노드 (리눅스 A, B)
```bash
# 1. 설정 스크립트 실행 (최초 1회)
./scripts/setup-worker.sh

# 2. 서비스 시작
cd deploy
docker compose -f docker-compose.worker.yml up -d
```

### 연결 확인

#### HDFS 클러스터 상태
```bash
# Master에서 실행
docker exec namenode hdfs dfsadmin -report
```

예상 출력:
```
Live datanodes (2):
  Name: 192.168.55.158:9866
  Name: 192.168.55.9:9866
```

#### Spark 클러스터 상태
```bash
# Spark Master UI 확인
http://192.168.55.114:8082
```

예상: Workers (2) 표시

### 단일 PC vs 분산 환경

| 구분 | 단일 PC (개발용) | 분산 환경 (운영용) |
|------|-----------------|------------------|
| **docker-compose 파일** | docker-compose.yml | docker-compose.master.yml + docker-compose.worker.yml |
| **HDFS 복제** | DataNode 2개 (같은 PC) | DataNode 2개 (다른 PC) |
| **Spark** | Master + Worker 1개 | Master + Worker 2개 |
| **장애 대응** | 불가 | 1대 장애 시 계속 운영 |
| **성능** | 리소스 경쟁 | 리소스 분산 |

### 주의사항

1. **네트워크**: 모든 PC가 같은 네트워크(192.168.55.x)에 있어야 함
2. **방화벽**: 필요한 포트가 열려있어야 함
3. **시간 동기화**: 모든 PC의 시간이 동기화되어야 함 (NTP 사용 권장)
4. **Master 먼저 시작**: Worker는 Master가 실행된 후 시작해야 연결됨

### 서비스 URL 요약

| 서비스 | URL | 용도 |
|--------|-----|------|
| Kafka UI | http://192.168.55.114:8080 | Kafka 토픽/메시지 모니터링 |
| HDFS | http://192.168.55.114:9870 | 분산 파일 시스템 관리 |
| Spark | http://192.168.55.114:8082 | Spark 작업 모니터링 |
| Airflow | http://192.168.55.114:8084 | 배치 작업 스케줄링 |
| Grafana | http://192.168.55.114:3000 | 메트릭 대시보드 |
| Prometheus | http://192.168.55.114:9090 | 메트릭 수집/쿼리 |
